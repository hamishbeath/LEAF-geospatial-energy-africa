...
class SEAR:
    ...
    model_filepath = '~/Library/Mobile Documents/com~apple~CloudDocs/Mitigation_project/SEAR_A/'
    ...
    global_carbon_tax = 'N'  # set to Y to include and N for no
    ...
    unmet_demand_penalty = 0.35  # $/kWh
    sensitivity_analysis = 'N'   # Set to 'Y' or 'N'
...
class SpatialData:
    spatial_filepath = SEAR.model_filepath + 'Spatial/'
...
def main() -> None:
    ...
    # population_access_per_year(SEAR.target_year)
    # annual_population_added_output(SEAR.target_year, SEAR.scenario)
    # return_households_added_by_cell(SEAR.scenario, SEAR.target_year)
    # for i in range(1, 5):
    #     electricity_demand_by_year(SEAR.target_year, SEAR.scenario, i)
    #     calculate_emissions_investment_national_grid(SEAR.target_year, SEAR.scenario, i)
    #     calculate_emissions_investment_off_grid(SEAR.target_year, SEAR.scenario, i, SEAR.reliability, 'MG')
    #     calculate_emissions_investment_off_grid(SEAR.target_year, SEAR.scenario, i, SEAR.reliability, 'SHS')
    #
    # for i in range(1, 5):
    #     return_least_cost_option(SEAR.target_year, SEAR.scenario, i, SEAR.reliability, SEAR.unmet_demand_penalty, SEAR.global_carbon_taxes)
    # Utils().calculate_grid_coverage_by_country()
    Utils().scenario_analysis(3, 'CT_Y_RP0.0', 'all')
    ...
    # Utils().household_installation_cost(SEAR.target_year)
...
def reliability_penalty_sensitivity_analysis():
    ...
    for i in range(2, 5):
        ...
        for penalty in reliability_penalties:
            ...
            tier_penalty_output = \
                return_least_cost_option(SEAR.target_year, SEAR.scenario, i, SEAR.reliability, unmet_demand_penalty)
            tier_penalty_output.index = [penalty]
            tier_results_output = pd.concat([tier_results_output, tier_penalty_output], axis=0)
            print(tier_results_output)
        tier_results_output.to_csv(SEAR.sensitivity_outputs
    ...
    for i in range(2, 5):
        ...
        for decile in carbon_prices.index:
            ...
            for year in range(0, 4):
                ...
                current_year += 5
            carbon_price_output = return_least_cost_option(SEAR.target_year, SEAR.scenario, i, SEAR.reliability,
                                                           SEAR.unmet_demand_penalty, decile_carbon_prices)
...
def population_access_per_year(target_year):
    ...
    baseline_summary['ISO3'], baseline_summary['Country'] = \
        SEAR.country_data_sheet['ISO3'], SEAR.country_data_sheet['location']
    #
    ...
    # Set country list values in summary sheet
    summary['ISO3'], summary['Country'] = SEAR.country_data_sheet['ISO3'], SEAR.country_data_sheet['location']
...
def return_households_added_by_cell(scenario, target_year):
    ...
    households_by_country, countries = SEAR.country_data_sheet['Household Size'], SEAR.country_data_sheet['ISO3']
...
def electricity_demand_by_year(target_year, scenario, tier_of_access):
    ...
    countries = SEAR.country_data_sheet['ISO3']
    energy_subs = SEAR.country_energy_subs['energy_file']
    ...
    for country in range(0, len(countries)):
        ...
        household_sum_dict = {}
        for year in range(2021, target_year + 1):
            # energy_file = (pd.read_csv(SEAR.CLOVER_load_filepaths + 'SSP' + str(SEAR.ssp) + '/Tier '
            ...
            energy_file = \
                (pd.read_csv('~/Library/Mobile Documents/com~apple~CloudDocs/Mitigation_project/CLOVER DEMAND BACKUP/'
                             + 'SSP' + str(SEAR.ssp) + '/Tier ' + str(tier_of_access) + '/' + str(lookup_country) +
                             '_community_load_' + str(year) + '.csv')[str(year)]) / 100
...
def calculate_emissions_investment_national_grid(target_year, scenario, tier_of_access):
    ...
    grid_costs = grid_distance_to * SEAR.grid_cost * grid_binary
    grid_construction_emissions = grid_distance_to * grid_binary * SEAR.extension_emissions_factor
    ...
    country_info = SEAR.country_data_sheet
...
def calculate_emissions_investment_off_grid(target_year, scenario, tier_of_access, reliability, system):
    ...
    To do:
        incorporate the calculation of the stand-alone systems
    """
    ...
    country_info = SEAR.country_data_sheet
    energy_info = SEAR.country_energy_subs
    ...
    try:
        pv_full_reliability = pd.read_csv(Utils.clover_results_filepath + 'pv_full_reliability_SSP_'
                                          + str(SEAR.ssp) + '_tier_' + str(tier_of_access) + '_' + system + '.csv')
        ...
        pv_full_reliability, diesel_full_reliability, pv_grid_reliability, diesel_grid_reliability = \
            Utils().return_relevant_clover_results_summary(ssp=SEAR.ssp, tier=tier_of_access, system=system)
    ...
    if reliability == 'full':
        ...
        print('SEAR.reliability needs to be set on either "full" or "grid"')
...
def return_least_cost_option(target_year, scenario, tier_of_access, reliability, reliability_penalty, carbon_prices):
    ...
    for cell in range(len(grid_existence)):
        ...
        if population_gained_access[cell] < 0.5:
            ...
            # Otherwise, test based on cost as to what the least-cost option is
                ...
                if population_gained_access[cell] >= SEAR.SHS_density_threshold:
                ...
                elif population_gained_access[cell] < SEAR.SHS_density_threshold:
    ...
    if SEAR.sensitivity_analysis == 'Y':
        ...
        summary_sheet.to_csv(SpatialData.spatial_filepath + 'cost_optimal_output_summary_' + reliability +
                             ...
                             + SEAR.global_carbon_tax + '_RP' + str(SEAR.unmet_demand_penalty) + '.csv')
...
def population_summaries():
    ...
    population_summary['ISO3'] = SEAR.country_data_sheet['ISO3']
    population_summary['Country'] = SEAR.country_data_sheet['location']
    ...
    population_summary.to_csv(SEAR.model_outputs + 'population_totals_by_year.csv')
